...
public class UserService {
    ...
    public void modifyUserName(PutUserReq putUserReq) throws BaseException {
        try {
            int result = userDao.modifyUserName(putUserReq); // 해당 과정이 무사히 수행되면 True(1), 그렇지 않으면 False(0)입니다.
    ...
    // 동네 수정
    public void modifyUserDong(PatchUserReq patchUserReq) throws BaseException {
        try {
            int result = userDao.modifyUserDong(patchUserReq); // 해당 과정이 무사히 수행되면 True(1), 그렇지 않으면 False(0)입니다.
            if (result == 0) { // result값이 0이면 과정이 실패한 것이므로 에러 메서지를 보냅니다.
                throw new BaseException(MODIFY_FAIL_USERDONG);
            }
        } catch (Exception exception) { // DB에 이상이 있는 경우 에러 메시지를 보냅니다.
            throw new BaseException(DATABASE_ERROR);
        }
    ...
    // 해당 userIdx를 갖는 User 탈퇴 (status 변경)
